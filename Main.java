 ...
 public class Main {
    ...
    private static void calc(){
        ...
        String secondParam = part[1].trim();
            if (part.length > 2) { throw new IllegalArgumentException("слагаемых больше двух");
        ...
        if (isArabian(firstParam) && isArabian(secondParam)) {
            result = calculated(arab1, arab2, operator);
            System.out.println("Результат");
            System.out.println(arab1 + " " + operator + " " + arab2 + " = " + result);

        } else if (isRoman(firstParam) && isRoman(secondParam)) {
            result = calculated(romanValue1, romanValue2, operator);
            if (result < 0) {
            ...
            System.out.println(firstParam + " " + operator + " " + secondParam + " = " + resultRoman);
